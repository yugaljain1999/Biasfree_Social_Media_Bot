...
############# EDIT importing praw - scrapper for reddit #########3
import praw
...
def main():
    ...
    if args.mode == "topic":
        ...
        if not args.gen_type == "gpt2":
            if args.gedi_model_name_or_path is None:
                args.gedi_model_name_or_path = "D:\ML_DL_AI Material\GeDi Model\GeDi\pretrained_models\gedi_topic"
    ...
    if args.mode == "sentiment":
        ...
        if not args.gen_type == "gpt2":
            if args.gedi_model_name_or_path is None:
                args.gedi_model_name_or_path = "D:\ML_DL_AI Material\GeDi Model\GeDi\pretrained_models\gedi_sentiment"
                if not os.path.isdir(args.gedi_model_name_or_path):
                    raise Exception("GeDi model path not found, must either run `get_models.sh' or set `args.gedi_model_name_or_path'")
    ...
    if args.mode == "detoxify":
        ...
        if not args.gen_type == "gpt2":
            if args.gedi_model_name_or_path is None:
                args.gedi_model_name_or_path = "D:\ML_DL_AI Material\GeDi Model\GeDi\pretrained_models\gedi_detoxifier"
                if not os.path.isdir(args.gedi_model_name_or_path):
                    raise Exception("GeDi model path not found, must either run `get_models.sh' or set `args.gedi_model_name_or_path'")
        if args.class_bias is None:
            args.class_bias = 2.0
    ...
    if args.gen_type == "gedi" :
        ...
        gedi_model=None
    torch.save(model,'AG_news_gedi_model_detoxify.pth')
